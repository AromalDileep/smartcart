// ----------------------------------------
// DYNAMIC BASE_URL LOADER
// ----------------------------------------
let BASE_URL = ""; // will be populated from backend
...
// initApp will hold the existing document-ready logic
function initApp() {
  // ----------------------------------------
  ...
  document.getElementById("approvedLoadMore").onclick = () =>
    loadApproved(false);
  ...
  async function loadFaissStats() {
    try {
      ...
      document.getElementById("stat_faiss_vectors").textContent =
        j.faiss_vectors;
    } catch (err) {
...
// fetch config and then initialize app
(async function loadConfigAndStart() {
  try {
    ...
    // start app (which attaches DOM handlers and performs initial loads)
    initApp();
  } catch (err) {
    console.error("Could not load config:", err);
    // Still call initApp so admin UI shows, but images may fallback
    initApp();
  }
})();
